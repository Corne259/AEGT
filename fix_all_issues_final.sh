...
echo "🚀 FIXING ALL AEGT ISSUES - FINAL COMPREHENSIVE FIX"
echo "=================================================="
...
sudo -u postgres psql -c "ALTER ROLE aegisum_user CREATEDB;" || true
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE aegisum_db TO aegisum_user;" || true
sudo -u postgres psql -c "GRANT ALL ON SCHEMA public TO aegisum_user;" || true
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO aegisum_user;" || true
sudo -u postgres psql -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO aegisum_user;" || true
sudo -u postgres psql -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO aegisum_user;" || true
...
if curl -f -s -k https://webapp.aegisum.co.za/health > /dev/null 2>&1; then
    echo "✅ Backend health check passed"
else
    echo "❌ Backend health check failed - checking if backend is running..."
    pm2 status
fi

# Test API endpoint
echo "Testing API endpoint..."
if curl -f -s -k https://webapp.aegisum.co.za/api/mining/status > /dev/null 2>&1; then
...
if curl -f -s -k https://webapp.aegisum.co.za > /dev/null 2>&1; then
